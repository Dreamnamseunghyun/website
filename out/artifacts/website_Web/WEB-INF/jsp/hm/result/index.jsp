                    ...
                    <div class="dx-viewport">
                        <div class="row-wrap row-wrap-box">
                            <div class="row-wrap logo-area-left btn-area-left">
                                <div class="row-box-title"><i class="xi-play-circle"></i>湲곗뾽泥대퀎 寃곌낵 寃??/div>
                                <div class="row-box">
                                    <div class="row-box-inner1">
                                        <div class="col-box1 row">
                                            <div class="col-lg-12 row">
                                                <div class="col-lg-box1 col-lg-box-vertical row margin-button">
                                                    <div class="col-lg-2 tag-name"><span>湲곗뾽泥?寃??/span></div>
                                                ...
                                                <div class="col-lg-box1 col-lg-box-vertical row margin-button">
                                                    <div class="col-lg-2 tag-name"><span>寃곌낵 ?깅줉??/span></div>
                                                    <div class="col-lg-5" id="resultDateBegin"
                                                         data-sitebuilder="dxDateBox"></div>
                                                    <div class="col-lg-5" id="resultDateEnd"
                                                         data-sitebuilder="dxDateBox"></div>
                                                </div>
                                            </div>
                                        </div>
...
<script>
    $(function () {
        ...
        function LoadCompanyResultList() {
            let data = {
        ...
        dxInstances.btnApply.option({
            text: '寃곌낵?깅줉',
            onClick: function () {
                location.href = '<c:url value="/hm/result/create"/>';
            },
        });
        ...
        dxInstances.resultDateBegin.option({
        ...
        dxInstances.resultDateEnd.option({
            applyValueMode: 'instantly',
            displayFormat: 'yyyy-MM-dd',
            type: 'date',
            onValueChanged: function (e) {
        ...
        dxInstances.dataGridCompanyResult.option({
            ...
            columns: [{
                ...
                cellTemplate: function (cellElement, cellInfo) {
                    cellElement.text(cellInfo.component.pageIndex() * cellInfo.component.pageSize() + cellInfo.row.rowIndex);
                },
            }, {
                //湲곗뾽泥대챸
                ...
                buttons: [{
                    ...
                    onClick: function (e) {
                        if (confirm('?뺣쭚濡???젣?섏떆寃좎뒿?덇퉴?')) {
                            $.ajax({
                                url: '<c:url value="/hm/result"/>/' + e.row.data.checkupResultId,
                                method: 'DELETE',
                            }).then(function () {
                                LoadCompanyResultList();
                            });
        ...
        $.when($.ajax(ajaxCompanyList)).then(function (responseCompanyList) {
            // Company List
            responseCompanyList.data.unshift({companyId: null, companyName: '?꾩껜'});
            dxInstances.lookupCompany.option({
        ...
        excelLoad();
        dxInstances.btnCompanyDownload.option({
            text: '寃곌낵?ㅼ슫濡쒕뱶',
            onClick: function () {
        ...
        function excelLoad() {
            $.ajax({
                url: '<c:url value="/hm/resulte-excel"/>',
                method: 'GET',
                data: {
                    companyId: dxInstances.lookupCompany.option('value'),
                    checkupYear: dxInstances.selectCompanyYear.option('value'),
                    resultDateBegin: dxInstances.resultDateBegin.option('value').toDateString(),
                    resultDateEnd: dxInstances.resultDateEnd.option('value').toDateString(),
                },
            }).then(function (response) {
