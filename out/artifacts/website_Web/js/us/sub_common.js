...
//?뚮뜑留??먯쓣???곹????덈뒗 ?곕꽆踰??섏씠??$("document").ready(function() {
    if($(".phone_num").length > 0) {
        $(".phone_num").val($(".phone_num").val().replace(/[^0-9]/g, "").replace(/(^02|^0505|^1[0-9]{3}|^0[0-9]{2})([0-9]+)?([0-9]{4})$/, "$1-$2-$3").replace("--", "-"));
    }
})
...
//泥댄겕諛뺤뒪(all_check)
// var $sportAll = $('.all_check input');
// $sportAll.change(function () {
//   var $this = $(this);
//   var checked = $this.prop('checked');
//   $('.agreement_area input').prop('checked', checked);
//
// });
//
//
// var check_length = $('.agreement_area input');
// check_length.change(function () {
//
//   var selectAll = true;
//   var count = check_length.length;
//   var i = 0;
//   for (; i < count; i++) {
...
var chk_active = $(this).children("input").addClass("active");
    if (chk){ //泥댄겕媛 ?섏뿀?꾩떆
        ...
        $(this).children("input").prop("checked",true).next().removeClass("chk_true1").next().removeClass("chk_true2");
    }
...
var chk_val = $(".check_box_value").val();
$(".chk_area input[value="+chk_val+"]").prop("checked", true);
$(".chk_type2 .chk_boxArea label").click(function(){
    var check_input_val = $(this).children("input").val();
    $(this).parents(".chk_area").find(".check_box_value").val(check_input_val);
});
...
window.onload = function () {
    if($(".board_wrap .pagination").length > 0) {
        var pagenation_active = $(".board_wrap .pagination strong").index();
        var pagenation_h = $(".board_wrap").offset().top;
        if (pagenation_active > 2) {
            $('html, body').stop().animate({scrollTop: pagenation_h}, 0);
        }
    }
}
