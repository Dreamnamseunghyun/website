...
import kr.co.sitebuilder.reserve.service.UserReserveService;
import kr.co.sitebuilder.reserve.vo.*;
                    ...
                    for (CheckupItemCategoryVO checkupItemCategory : checkupItemClass.getCheckupItemCategories()) {
                        for (CheckupItemVO checkupItem : checkupItemCategory.getCheckupItems()) {
                            if (vo.getItemIdList().contains(checkupItem.getItemId())) {
                                selectedItemMessages.append("???좏깮?섏떊 蹂묒썝? 怨좉컼?섍퍡???щ쭩?섏떊 ").append(checkupItem.getItemName()).append(" 寃?ш? ").append(checkupRegister.getRegisterName()).append("???ы븿?섏뼱 ?덉뒿?덈떎.<br/>");
