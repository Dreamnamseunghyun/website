...
<script>
    $(function () {
        ...
        <%--    type: 'success',--%>
        <%--    onClick: function () {--%>
        <%--        location.href = '<c:url value="/cm/reserve/${reservationId}"/>' + '/edit';--%>
        <%--    }--%>
        <%--});--%>
        ...
        dxInstances.btnList.option({
            ...
            onClick: function () {
                if (dxInstances.reservationState.option('value') === 0) {
                    location.href = '<c:url value="/cm/cancel-reserve"/>';
                } else {
                    location.href = '<c:url value="/cm/reserve"/>';
                }
            }
        });
        ...
        // Load
        $.ajax({
            url: '<c:url value="/cm/reserve/${reservationId}"/>',
            method: 'GET',
        ...
        function LoadManagerList() {
            $.ajax({
                url: '<c:url value="/cm/reserve/manager"/>',
                method: 'GET',
                data: {
